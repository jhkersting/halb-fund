...
# if changing cwd gives error
try(setwd("Ch 3 Quantitative Finance Tools/3.5 Sorting Data"), silent = TRUE)
# set wd to location of this file
# Inputs
...
SortData$Day <- trunc(SortData$Date -
                        SortData$Year * 10000 -
                        SortData$Month * 100)
...
hist(y, main = mTitle, breaks = 50, freq = FALSE, col = "black", labels = FALSE,
     sub = sTitle, xlab = xTitle, ylab = yTitle, plot = TRUE, axes = TRUE, density = 10)
curve(dnorm(x, yMean, yStdDev), col = "black", lwd = 4)
