...
# CHnage the working directory to the folder where the data is stored
setwd("Ch 3 Quantitative Finance Tools/3.4 LSC Curve Fitting Model")
cwd <- getwd()
# Libraries
...
Scalars <- c(1:N)
Scalars[1] <- 2.0
NumberOfDates <- 5 #CMT data were collected over 5 points in time
# Read in data from file
# NumberOfMaturities <- 9
DataSource <- 'UST'
# LSCData <- read.table("LSCInputData.dat",header = FALSE)
NumberOfMaturities <- 8
LSCData <- read.table("USTInputData.dat",header = FALSE) #list
#typeof(LSCData)
NumberOfDates <- length(LSCData$V1) - 1 # because it is a list; subtract 1 for the row of column names
