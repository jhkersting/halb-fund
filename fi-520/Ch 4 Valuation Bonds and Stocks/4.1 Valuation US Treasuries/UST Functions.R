...
# Decrement maturity date until it is a valid date (e.g., 2/31/YYYY)
    ...
    if (Frequency == 2) { # Semi-annual
      ...
      # While-loop ends when (JMaturityDate <= JSettlementDate); "JMaturityDate" has been decremented to the last "coupon date"
      # Now, we calculate the next coupon date "JNextMaturityDate"
      if(MM > 6){
        MM = MM - 6
        MY = MY + 1
      } else {
