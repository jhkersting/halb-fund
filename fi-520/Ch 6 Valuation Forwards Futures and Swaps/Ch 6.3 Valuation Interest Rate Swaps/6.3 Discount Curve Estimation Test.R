...
# # Small input data set
    if(i==1)MarketSwapRates[i] <- 4
    if(i==2)MarketSwapRates[i] <- 3.5
    if(i==5)MarketSwapRates[i] <- 3
    if(i==10)MarketSwapRates[i] <- 4.5
    if(i==20)MarketSwapRates[i] <- 5
    if(i==30)MarketSwapRates[i] <- 5
...
MarketSwapRates
# Scalars: Taus
NTau <- 5   # Must be between 0 and 5 (not inclusive, integer)
