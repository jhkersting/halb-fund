...
#
# BondValue: Dollar value of bond including accrued interest 
#  based on LSC model fit of both the base curve and spread curve
#
BondValueDF2 = function(B, BC, SC){
  with(B,{
    PV = 0.0
    RemainingCoupons = CouponsRemaining(B)
    ElapsedTime = Elapsed(B)
    ...
    for(i in 1:RemainingCoupons){
      ...
      if(i == 1){
        ...
        BC$Maturity <- ((i - ElapsedTime)/2.0) #- 0.5
      }
      BaseRate <- LSCRate(BC)
      SpreadRate <- LSCRate(SC)
      PV <- PV + CF*exp(-((BaseRate + SpreadRate)/100)*BC$Maturity)
    }
    return( PV )
  })
}
