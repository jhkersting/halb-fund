...
# Input Scalars: Taus for CMT
NBCTau <- NBCFactors - 2   # Must be between 0 and 7 (not inclusive, integer)
BCTau <- numeric(NBCTau)   # b (Level, slope, and curvatures)
#
# Input set of Taus
#
BCTau[1] <- 2.0 # Overwritten with bond maturity in test program
if(NBCTau>1)BCTau[2] <- 3.0
if(NBCTau>2)BCTau[3] <- 6.0
if(NBCTau>3)BCTau[4] <- 15.0
if(NBCTau>4)BCTau[5] <- 0.5
if(NBCTau>5)BCTau[6] <- 4.0
# CMT test inputs
